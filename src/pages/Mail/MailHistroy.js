...
class MailHistroy extends Component {
 ...
 state={info:[] ,titleModal:false,title:'',contentModal:false,content:'',attachmentModal:false,attachment:{}}
  ...
  handleSubmit = v => {
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        ...
        console.log(this.props.form.getFieldValue('sendtime')[0].valueOf())
        console.log(this.props.form.getFieldValue('sendtime')[1].valueOf())
        var table = []
        for (let i = 0; i < 10; i++) {
          table.push({
            time: Date.now(),
  ...
  render() {
    const data = [
        ...
        },
        {
          ...
          key: 'title',
          render:(text, record)=> {
            return <Button onClick={()=>{this.handleTitleClick(record)}}> 点击查看</Button>
          },
        },
        {
          title: '邮件内容',
          ...
          key: 'content',
          render:(text, record)=> {
            return <Button onClick={()=>{this.handleContentClick(record)}}> 点击查看</Button>
          },
        },
        {
          title: '附件内容',
          ...
          key: 'attachment',
          render:(text, record)=> {
            return <Button onClick={()=>{this.handleAttachmentClick(record)}}> 点击查看</Button>
          },
        },
      ]
    ...
    return (
      <div>
        ...
        <Modal
          ...
          onOk={()=>{this.setState({titleModal:false})}}
          onCancel={()=>{this.setState({titleModal:false})}}
        >
          {this.state.title}
        </Modal>
        </div>
        ...
        <Modal
          ...
          onOk={()=>{this.setState({attachmentModal:false})}}
          onCancel={()=>{this.setState({attachmentModal:false})}}
        >
